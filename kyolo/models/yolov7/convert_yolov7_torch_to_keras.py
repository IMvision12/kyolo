"""Convert an official YOLOv7 PyTorch checkpoint into kyolo Keras 3 weights.
...
Usage::
    ...
    python -m kyolo.models.yolov7.convert_yolov7_torch_to_keras \
        --weights yolov7.pt --output yolov7.weights.h5 --variant ''
"""
...
from .yolov7_model import build_yolov7
...
def convert(
    ...
    """Build a YOLOv7 model and transfer weights from ``weights_path``."""
    model = build_yolov7(variant=variant, nc=nc, input_shape=(imgsz, imgsz, 3), deploy=True)
...
def main():
    ...
    p = argparse.ArgumentParser(description="Convert YOLOv7 .pt -> kyolo .weights.h5")
    p.add_argument("--weights", required=True, help="Path to the source .pt checkpoint")
    p.add_argument("--variant", default="", help="Model variant")
