...
pip install -e ".[convert]"  # torch + ultralytics, for weight conversion
...
from kyolo.models import get_model
...
model = get_model("yolov8n", nc=80, input_shape=(640, 640, 3), deploy=True)
...
official `.pt` file yourself:

```bash
python scripts/convert.py \
    --model yolov8n \
    --weights yolov8n.pt \
    --output yolov8n.weights.h5
```

Conversion requires the `convert` extra (`pip install -e ".[convert]"`, which
pulls in `torch` and `ultralytics`). The programmatic entry point is
...
│   └── train.py            # minimal synthetic training demo
├── scripts/
│   └── convert.py          # CLI wrapper around kyolo.conversion
├── src/kyolo/
│   ├── models/             # get_model + family factories (YOLOv8, ...)
│   ├── heads/              # detection heads
│   ├── layers/             # conv/bn primitives, blocks, DFL, letterbox, ...
│   ├── ops/                # anchors + box math (pure keras.ops)
│   ├── losses/             # YOLODetectionLoss
│   ├── preprocessing/      # YOLOPreprocessor
│   ├── postprocessing/     # YOLOPostprocessor
│   ├── training/           # YOLODetector (keras.Model subclass)
│   ├── conversion/         # convert_weights + CLI
│   └── utils/              # COCO metadata + visualization
└── tests/                  # build + processing smoke tests
```
