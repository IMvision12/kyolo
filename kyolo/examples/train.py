...
def parse_args():
    ...
    parser.add_argument("--model", default="yolov8n", help="Model name for get_model.")
...
def build_detector(model_name, nc, weights=None, freeze_backbone=False):
    ...
    Fine-tuning notes:
      * Load converted weights onto the *bare* model BEFORE wrapping it in the
        ...
        (see the README / ``scripts/convert.py``); they are not shipped here.
    ...
    model = get_model(model_name, nc=nc, input_shape=(IMAGE_SIZE, IMAGE_SIZE, 3), deploy=False)
