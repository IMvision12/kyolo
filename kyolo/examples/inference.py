...
predictions, convert an official checkpoint first (see ``scripts/convert.py``)
and pass it with ``--weights``.
...
def parse_args():
    ...
    parser.add_argument(
        "--model",
        default="yolov8n",
        help="Model name understood by kyolo.models.get_model (e.g. yolov8n).",
    )
    parser.add_argument(
        "--weights",
        default=None,
...
def main():
    ...
    if args.weights is not None:
        ...
        print(
            ...
            "        python scripts/convert.py --model {m} --weights {m}.pt "
            "--output {m}.weights.h5\n"
