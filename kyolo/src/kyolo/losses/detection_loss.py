...
def _bce_with_logits(logits, targets):
    ...
    return (
        ops.maximum(logits, 0.0)
        - logits * targets
        + ops.log1p(ops.exp(-ops.abs(logits)))
    )
...
class YOLODetectionLoss:
    ...
    def __init__(
        self, nc=80, reg_max=16, strides=(8, 16, 32),
        box_gain=7.5, cls_gain=0.5, dfl_gain=1.5, tal_topk=10, use_dfl=True,
    ):
        self.nc = nc
