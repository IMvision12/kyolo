...
class Letterbox(keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        ratio = ops.broadcast_to(
            ops.expand_dims(ops.stack([r, r], 0), 0), [batch, 2]
        )
        pad = ops.broadcast_to(
            ops.expand_dims(ops.stack([dw_half, dh_half], 0), 0), [batch, 2]
        )
    ...
    def _fill_border(self, padded, top, bottom, left, right):
        b, h, w, c = ops.shape(padded)[0], ops.shape(padded)[1], ops.shape(padded)[2], ops.shape(padded)[3]
