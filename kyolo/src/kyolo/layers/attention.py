...
def psa_block(x, num_heads=4, attn_ratio=0.5, shortcut=True, data_format="channels_last", name="psablock"):
    """YOLO11 PSABlock: residual attention followed by residual FFN."""
    c = channels_of(x, data_format)
    a = mhsa(x, num_heads=num_heads, attn_ratio=attn_ratio, data_format=data_format, name=f"{name}.attn")
...
def c2psa(x, c2, n=1, e=0.5, data_format="channels_last", name="c2psa"):
    ...
    for i in range(n):
        b = psa_block(b, num_heads=num_heads, attn_ratio=0.5, data_format=data_format, name=f"{name}.m.{i}")
...
def scdown(x, c2, kernel_size=3, strides=2, data_format="channels_last", name="scdown"):
    ...
    return conv_bn(
        y, c2, kernel_size, strides, groups=c2, act=False,
        data_format=data_format, name=f"{name}.cv2",
    )
...
def a2c2f(
    x, c2, n=1, area=1, num_heads=4, mlp_ratio=1.2, e=0.5,
    data_format="channels_last", name="a2c2f",
):
    ...
    cur = y
    for i in range(n):
        att = area_attention(cur, num_heads=num_heads, area=area, data_format=data_format, name=f"{name}.m.{i}.attn")
