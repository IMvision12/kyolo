...
Exposed as the ``kyolo-convert`` console script (see ``pyproject.toml``) and
wrapped by ``scripts/convert.py``. Builds the requested kyolo model in deploy
(inference) form, then transfers weights from a user-supplied ``.pt`` file.
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if verbose:
        print(
            f"Building {args.model} (nc={args.nc}, imgsz={args.imgsz}, deploy=True) ..."
        )
    model = get_model(
        args.model,
        nc=args.nc,
